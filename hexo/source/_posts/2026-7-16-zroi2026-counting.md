...
强连通不好计数，因为强连通是个连接非常紧密的东西，考虑正难则反，如果他不是强连通图，那么缩点以后应该是个 DAG，所以只要对 DAG 进行计数就好了。设 $f(S)$ 表示 $S$ 集合的答案，$g(S)$ 表示将 $S$ 内的点划分成若干个 SCC 的方案数。但是问题是，后面容斥的时候，容斥系数和连通块的数量是相关的，于是需要把容斥系数乘进去，令 $g(S)$ 表示划分成奇数个 SCC 的方案数减去偶数个 SCC 的方案数。

然后就可以写容斥了，有：

$$
f(S) = 2^{E(S,S)}-\sum_{T \subseteq S,T \neq \varnothing} g(S) 2^{E(T,S-T)+E(S-T,S-T)}
$$

然后问题就是如何求 $g$ 了。考虑每次加入一个 SCC，钦定 $\operatorname{lowbit}(S)$ 所在的 SCC 是新加入的，那么容易得到转移：

$$
g(S) = f(S) - \sum_{T \subset S,\operatorname{lowbit}(S)=\operatorname{lowbit}(T)}f(T)g(S-T)
$$

一个小问题是这里似乎存在相互依赖的转移，但是并没有，因为 $f$ 删掉的是不合法的情况，只有一个 SCC 是合法的，所以先在没有 $f$ 的情况下转移一个 $g$，然后用这个 $g$ 转移 $f$，再把 $f$ 加上即可。

现在问题就是如何求 $E$。考虑一个类似于 DP 的思路，动态往起点侧加点即可。

## [[ARC105F] Lights Out on Connected Graph](https://atcoder.jp/contests/arc105/tasks/arc105_f)
数二分图比较困难，可以考虑数二分图黑白染色方案数。

由于图是连通的，只需要把所有染色以后合法的方案都算出来，然后除以 $2$ 就是答案。但是连通更不好刻画，于是考虑正难则反，先算出所有黑白染色对应二分图的方案数，再把不连通的都删掉。首先所有黑白染色可以直接子集枚举，算出来 $S$ 集合得到的二分图数量为 $g(S)$，然后设 $f(S)$ 为得到的合法二分图的数量，钦定 $\operatorname{lowbit}(S)$ 所在的连通块是新加入的，枚举他所在的连通块，删掉就可以了。

## [Reunion](https://codeforces.com/problemset/problem/1517/F)
转成数 $\ge k$ 的方案数再加起来。

那么容易用一个树上 DP 做，考虑当前子树有两种情况：完全被覆盖，这种情况下他还可以向上传递；根节点还没有被覆盖，并且可能子树中还有节点。那么设 $f_{i, j, 0/1}$ 表示 $i$ 为根的子树，是哪种情况以及深度是多少。然后转移的时候可以做一个类似于树上背包的事情做就可以了。

## [「MCOI-06」Distinct Subsequences](https://www.luogu.com.cn/problem/P7888)
考虑对于一个字符串的总的价值是怎么算的。记录 $f(c)$ 表示以 $c$ 结尾的字符串总数，然后扫字符串，每次扫到一个字符就把 $f(c) = \sum f(x) + 1$。

这样的话，考虑子序列的价值和是多少。如果转期望的话，就相当于是有一半的概率选或不选，然后转移一下，最后乘以总数即可。

## [Segment Covering](https://codeforces.com/problemset/problem/1774/G)
很有意思的问题。

首先考虑相互包含的线段，一个关键的观察是，这种情况下包含的线段（不是被包含的）可以被删掉。原因是选了包含的线段以后，选不选被包含的线段恰好会提供正负完全相等的贡献，然后就和没有一样。

这样就没有相互包含的线段了。然后开始推断，从一个 $l$ 开始，如果有三条线段满足 $l_1 < l_2 < l_3 < r_1$，那么第三条线段可以被删除，原因同上。然后从头推理一遍，最后肯定剩下的要么必须选，要么不能选，然后判断他们的并就可以了。

然后对于多组询问，把上面的过程加上一个倍增就好了。不难发现答案的绝对值不超过 $1$。